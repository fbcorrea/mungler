...
#SBATCH -J mungler              # the job name, just like on UGE, can be hard coded here
#SBATCH -t 1:00:00             # maximal runtime (hour:minutes:seconds)
#SBATCH --mem-per-cpu 24G       # amount of memory per core
...
QUERY=${3:-"Which animals were playing an excentric game? Be specific and brief."}

# Record the start date and time
date

echo "Starting ollama Singularity container..."
...
echo "Executing MunGLER v0.1.0 script..."
...
/gpfs1/work/borimcor/mungler/mungler \
	--query_text="$QUERY" \
	--model_llm="$MODEL" \
	--model_embedding="$EMBMODEL"

#nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits >> /work/borimcor/mungler/gpu_memory_usage.log
